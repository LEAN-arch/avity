...
def generate_spc_data(batch_id, parameter='Oligo Concentration'):
    np.random.seed(hash(batch_id) % (2**32 - 1)); n_points = 20
    mean = 10.0 if parameter == 'Oligo Concentration' else 7.2; std_dev = 0.2 if parameter == 'Oligo Concentration' else 0.05
    lsl = 9.5 if parameter == 'Oligo Concentration' else 7.0; usl = 10.5 if parameter == 'Oligo Concentration' else 7.4
    data = np.random.normal(mean, std_dev, n_points)
    if batch_id == 'AVC-DM1-P3-B006': data[15:] -= np.linspace(0, 0.4, 5)
    if batch_id == 'AVC-DM1-P3-B007': data[10:] = np.random.normal(mean - 0.5, std_dev*1.5, 10); data[18] = lsl - 0.1
...
def generate_quality_data():
    return pd.DataFrame({'Record ID': ['DEV-24-015', 'CAPA-23-008', 'CR-24-031', 'DEV-24-018'],'CDMO': ['Catalent Pharma', 'WuXi Biologics', 'Fujifilm Diosynth', 'Catalent Pharma'],'Type': ['Deviation', 'CAPA', 'Change Request', 'Deviation'],'Description': ['Temperature excursion during upstream process step.', 'Implement new training for analytical method transfer.', 'Update master batch record with new stirring parameters.', 'Contamination event led to batch failure.'],'Status': ['Investigation', 'Effectiveness Check', 'Pending Approval', 'Root Cause Analysis'],'Priority': ['High', 'Medium', 'Low', 'Critical'],'Days Open': [15, 92, 5, 2]})
...
def generate_tech_transfer_data():
    ...
    data = {
        ...
        'Progress (%)': [100, 100, 100, 100, 75, 20, 0] # NEW: Progress data
    }
    df = pd.DataFrame(data)
